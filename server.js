...
app.use(cors({
  origin: process.env.NODE_ENV === 'production' 
    ? (process.env.FRONTEND_URL || true) // 在生产环境中，前后端同域，允许所有源
    : ['http://localhost:3000', 'http://localhost:5001'], // 开发环境允许前后端端口
  credentials: true
}));
  ...
  try {
    for (const student of students) {
      try {
        let classResult;
        if (usePostgres) {
          classResult = await db.query('SELECT id FROM classes WHERE name = $1', [student.className]);
        } else {
        ...
        let classId = usePostgres ? classResult.rows[0]?.id : classResult?.id;
        
        if (!classId) {
          // 自動創建班別
          if (usePostgres) {
            const newClassResult = await db.query('INSERT INTO classes (name) VALUES ($1) RETURNING id', [student.className]);
        ...
        if (usePostgres) {
          await db.query('INSERT INTO students (class_id, student_number, name_zh, name_en) VALUES ($1, $2, $3, $4)', 
            [classId, student.studentNumber, student.nameZh, student.nameEn]);
        } else {
          await new Promise((resolve, reject) => {
            db.run('INSERT INTO students (class_id, student_number, name_zh, name_en) VALUES (?, ?, ?, ?)', 
              [classId, student.studentNumber, student.nameZh, student.nameEn], function(err) {
              if (err) reject(err);
              else resolve();
            });
          });
...
app.get('/api/leaderboard', async (req, res) => {
  ...
  try {
    ...
    if (sport_id) {
      conditions.push(usePostgres ? 'r.sport_id = $' + (params.length + 1) : 'r.sport_id = ?');
      params.push(sport_id);
    }
    if (class_id) {
      conditions.push(usePostgres ? 's.class_id = $' + (params.length + 1) : 's.class_id = ?');
      params.push(class_id);
    }
    ...
    query += ' ORDER BY r.time_min ASC, r.time_sec ASC';
...
// 新增成績（含拍照）
app.post('/api/results', upload.single('photo'), async (req, res) => {
  ...
  try {
    ...
    let existingResult;
    if (usePostgres) {
      const result = await db.query('SELECT * FROM results WHERE student_id = $1 AND sport_id = $2', [student_id, sport_id]);
      existingResult = result.rows[0];
    } else {
      existingResult = await new Promise((resolve, reject) => {
    ...
    if (existingResult) {
      const existingTimeTotal = existingResult.time_min * 60 + existingResult.time_sec;
      if (newTimeTotal < existingTimeTotal) {
        // 新成績更快，更新記錄
        if (usePostgres) {
          await db.query('UPDATE results SET time_min = $1, time_sec = $2, photo_path = $3, created_at = CURRENT_TIMESTAMP WHERE student_id = $4 AND sport_id = $5', 
            [time_min, time_sec, photo_path, student_id, sport_id]);
        } else {
          await new Promise((resolve, reject) => {
      ...
      // 新增成績
      if (usePostgres) {
        const result = await db.query('INSERT INTO results (student_id, sport_id, time_min, time_sec, photo_path) VALUES ($1, $2, $3, $4, $5) RETURNING *', 
          [student_id, sport_id, time_min, time_sec, photo_path]);
        res.json(result.rows[0]);
      } else {
        await new Promise((resolve, reject) => {
